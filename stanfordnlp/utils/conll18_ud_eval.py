        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    if not treebank_type.get('cycles_okay', False):
            ...
            if not treebank_type.get('no_root_okay', False):
...
def evaluate_wrapper(args):
    ...
    treebank_type['no_root_okay'] = args.no_root_okay
    treebank_type['cycles_okay'] = args.cycles_okay
...
def main():
    ...
    parser.add_argument('--no-root-okay', default=False, action='store_true',
                        help='A sentence can have no nodes with HEAD=0.')
    parser.add_argument('--cycles-okay', default=False, action='store_true',
                        help='A sentence can have cycles.')
